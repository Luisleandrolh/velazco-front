...
export class DashboardComponent implements OnInit {
  ...
  weeklyChartOptions: ChartOptions = {
    responsive: true,
    plugins: {
      title: { display: true, text: "Ventas Semanales (último mes)", font: { size: 18 } },
      tooltip: {
        callbacks: {
          label: (ctx) => {
  ...
  salesByDayDetails: { date: string; salesCount: number; totalSales: number }[] = []
  ...
  private loadDailySales(): void {
    this.dashboardService.getDailySales().subscribe((data) => {
      ...
      this.localProductSummary = Object.entries(resumenMap).map(([productName, info]) => ({
        productName,
        ...info,
      }))
  ...
  private loadWeeklySales(): void {
    this.dashboardService.getWeeklySalesDetails().subscribe((weeks) => {
      ...
      this.weeklyTooltipData = weeks.map((w, i) => {
        ...
        const [topDay, topSum] = Object.entries(daySums).sort((a, b) => b[1] - a[1])[0] || ["-", 0]
  ...
  private loadTopProducts(): void {
    this.dashboardService.getTopProducts().subscribe({
      ...
      error: (error) => {
        console.warn("Error loading top products from endpoint, using local calculation:", error)
  ...
  getProductSummary(): any[] {
    return this.resumenProductos.length > 0 ? this.resumenProductos : this.localProductSummary
  }
}
